(function($) {
    $.fn.weekly_schedule = function(callerSettings) {
        var settings = $.extend({
            days: ["星期一", "星期二", "星期三", "星期四", "星期五", "星期六", "星期天"], // Days displayed
            hours: "0:00-23:00", // Hours displyed
            fontFamily: "Montserrat", // Font used in the component
            fontColor: "#606266", // Font colot used in the component
            fontWeight: "100", // Font weight used in the component
            fontSize: "14px", // Font size used in the component
            hoverColor: "#f6f7fa", // Background color when hovered
            selectionColor: "#5ca96d", // Background color when selected
            ...
            for (var i = 0; i < dayContainer.length; i++) {
                ...
                for (var j = 0; j < children.length; j++) {
                    if ($(children[j]).hasClass('selected')) {
                        ...
                        let endTime = '';
                        let startTime = children[j].classList[1];
                        let num = startTime.split(":");
                        ...
                        endTime = num + ':00:00'
                        hoursSelected.push(startTime + '-' + endTime);
                    }
                }

                output.push(hoursSelected);
            }
            return output;
        }
        ...
        function setSelectedHour() {
            var dayContainer = $('.day');
            var selectedHours = JSON.parse(localStorage.getItem('selectedHours'));
            ...
            for (var i = 0; i < dayContainer.length; i++) {
                var children = $(dayContainer[i]).children();
                var selectedChildren = selectedHours[i];

                if(selectedChildren.length > 0) {
                    for(var s = 0; s < selectedChildren.length; s++) {
                        for (var j = 0; j < children.length; j++) {
                            var className = $(children[j]).attr("class");
                            var time = className.split(" ")[1];

                            if (time == selectedChildren[s]) {
        ...
        function parseHours(string) {
            ...
            for (curHour; curHour <= endHour; curHour++) {
                var parsedStr = curHour.toString() + ':00:00';
